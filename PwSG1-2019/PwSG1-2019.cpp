...
struct Tile { int val = 0; bool can_fuse = true; int bsize = 0; };
Score score;
Tile tiles[16];
...
void SaveToFile();
bool ReadFromFile();
void animateFuse(int n);
void animateNew(int n);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   m_hWnd[0] = CreateWindowW(szWindowClass, szTitle, WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX,
      xc-590, yc-200, crc.right-crc.left, crc.bottom-crc.top,nullptr,nullptr, hInstance, nullptr);
   m_hWnd[1]= CreateWindowW(szWindowClass, szTitle, WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX | WS_VISIBLE ,
       xc+290, yc-200, crc.right - crc.left, crc.bottom - crc.top, m_hWnd[0], nullptr, hInstance, nullptr);
    ...
    case WM_PAINT:
        {
            ...
            HPEN pen = CreatePen(PS_SOLID, 1, RGB(250, 247, 238));
             ...
             Rectangle(hdc, 0, 0, 290, 360);
             HBRUSH brush = CreateSolidBrush(RGB(204, 192, 174));
             SelectObject(hdc, brush);
             RoundRect(hdc, 10, 10, 280, 70, 20, 20);
             ...
             DrawText(hdc, buf/*&itoa(score.val) */, -1, &rc, DT_CENTER| DT_NOCLIP | DT_VCENTER | DT_SINGLELINE);
             ...
             for(int i=0;i<4;i++)
                 for (int j = 0; j < 4; j++)
                 {
                     ...
                     {
                     case 0: 
                            SelectObject(hdc, CreateSolidBrush(RGB(204, 192, 174)));    
                       break;
                     case 2: SelectObject(hdc, CreateSolidBrush(RGB(238, 228, 198)));
                         break;
                     case 4: SelectObject(hdc, CreateSolidBrush(RGB(239, 225, 218)));
                         break;
                     case 8: SelectObject(hdc, CreateSolidBrush(RGB(243, 179, 124)));
                         break;
                     case 16: SelectObject(hdc, CreateSolidBrush(RGB(246, 153, 100)));
                         break;
                     case 32: SelectObject(hdc, CreateSolidBrush(RGB(246, 125, 98)));
                         break;
                     case 64: SelectObject(hdc, CreateSolidBrush(RGB(247, 93, 60)));
                         break;
                     case 128: SelectObject(hdc, CreateSolidBrush(RGB(237, 206, 116)));
                         break;
                     case 256: SelectObject(hdc, CreateSolidBrush(RGB(239, 204, 98)));
                         break;
                     case 512: SelectObject(hdc, CreateSolidBrush(RGB(243, 201, 85)));
                         break;
                     case 1024: SelectObject(hdc, CreateSolidBrush(RGB(238, 200, 72)));
                         break;
                     case 2048: SelectObject(hdc, CreateSolidBrush(RGB(239, 192, 47)));
                         break;
                     default:
                         throw EXCEPTION_BREAKPOINT;
                     }
                     RoundRect(hdc, 70*j+10- tiles[ij2n(i, j)].bsize, 70 * i + 80- tiles[ij2n(i, j)].bsize, 70+j*70+ tiles[ij2n(i, j)].bsize, 70*i+140+ tiles[ij2n(i, j)].bsize, 20, 20);
                     if(tiles[ij2n(i,j)].val!=0)
                     {_stprintf_s(buf, bufSize, _T("%d"), tiles[ij2n(i, j)].val);
                     rc = { 70 * j + 10, 70 * i + 80, 70 + j * 70, 70 * i + 140 };
                     DrawText(hdc, buf, -1, &rc, DT_CENTER | DT_NOCLIP | DT_VCENTER | DT_SINGLELINE); }
                 }
             if (gameState != 0)
             {
                
                 HDC memDC = CreateCompatibleDC(hdc);
                 HBITMAP bitmap = CreateCompatibleBitmap(hdc, 290, 360); //LoadBitmap(hInst,MAKEINTRESOURCE(IDB_BITMAP1));
                 HBITMAP oldBitmap = (HBITMAP)SelectObject(memDC, bitmap);
                 if(gameState==1)
                    SelectObject(memDC, CreateSolidBrush(RGB(0,255,0)));
                 else if(gameState==2)
                     SelectObject(memDC, CreateSolidBrush(RGB(255,0, 0)));
                 Rectangle(memDC, 0, 0,290 ,360);
                 BLENDFUNCTION blend = { AC_SRC_OVER,0,128,0 };
                  AlphaBlend(hdc, 0,0, 290,360, memDC,0, 0,48,48, blend);
        ...
        case WM_TIMER:
             {
            if (timerCount == 1)
                SetTimer(m_hWnd[1], 16, 15, NULL);
                 if (wParam >= 0 && wParam<=15) // check timer id
                    {
                     if (tiles[wParam].bsize < 0)
                         tiles[wParam].bsize += 5;
                     else if (tiles[wParam].bsize > 0)
                         tiles[wParam].bsize -= 1;
                     else if (tiles[wParam].bsize == 0)
                     {
                         timerCount--;
                         KillTimer(m_hWnd[0], wParam);
                     }              
                      }
                 if (wParam == 16)
                 {
                     ...
                     UpdateWindow(m_hWnd[0]);
                     UpdateWindow(m_hWnd[1]);
                    if (timerCount==0) KillTimer(m_hWnd[1], wParam);
                 }
                 }
 break;
